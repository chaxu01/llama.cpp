...
static size_t detect_num_smcus() {
    // If the CPU doesn't support SME, there are no SMCUs to use.
    if (!ggml_cpu_has_sme()) {
        return 0;
    }

#if defined(__linux__) && defined(__aarch64__)
    // Linux/aarch64: Best-effort count of SME compute units (SMCUs) via SMIDR_EL1 sysfs.
    // Returns 0 if SMIDR_EL1 is not exposed/readable.
    ...
    for (size_t cpu = 0;; ++cpu) {
        ...
        if (!file.is_open()) {
            break; // stop at first missing CPU entry / non-exposed SMIDR
        }
    ...
    // macOS/Apple Silicon: No public SMIDR_EL1 interface.
    // Use a conservative default for SME-capable systems, with a small heuristic
    ...
    // SME is supported (checked above) but we couldn't determine an exact count.
    // Return a conservative non-zero value to allow "auto" SME enablement.
    return 1;

#else
    // SME supported (checked above) but platform-specific SMCU counting is unavailable.
    // Return a conservative default and rely on GGML_KLEIDIAI_SME=<n> for overrides.
    return 1;
#endif
}

static int parse_int_env(const char *s, const char *name, bool *ok) {
...
static void init_kleidiai_context(void) {
    ...
    if (!initialized) {
        ...
        if (env_threads) {
            bool ok = false;
            int hint = parse_int_env(env_threads, "GGML_TOTAL_THREADS", &ok);
            if (ok && hint > 0) {
        ...
        if (!cpu_has_sme) {
            if (sme_env_set) {
                // If user tried to force SME on a non-SME CPU, warn once.
                bool ok = false;
                int req = parse_int_env(env_sme, "GGML_KLEIDIAI_SME", &ok);
                if (ok && req > 0) {
            ...
            if (sme_env_set) {
                bool ok = false;
                int v = parse_int_env(env_sme, "GGML_KLEIDIAI_SME", &ok);
                sme_env_ok = ok;

                if (!ok) {
                    // Treat invalid env like "unset" => auto
                    detected_smcus = detect_num_smcus();
                    sme_cores = detected_smcus > 0 ? (int)detected_smcus : 0;
                } else if (v == 0) {
                    sme_cores = 0; // forced off
                } else { // v > 0
                    sme_cores = v; // forced N
                }
            } else {
                // Auto mode
                detected_smcus = detect_num_smcus();
        ...
        if (!ctx.kernels_q4) {
            GGML_LOG_WARN("kleidiai: no compatible q4 kernels found for CPU features mask %d\n", (int)ctx.features);
        } else {
            GGML_LOG_INFO("kleidiai: primary q4 kernel feature %s\n", cpu_feature_to_string(ctx.kernels_q4->required_cpu));
        }

        if (!ctx.kernels_q8) {
            GGML_LOG_DEBUG("kleidiai: no compatible q8 kernels found for CPU features mask %d\n", (int)ctx.features);
        } else {
            GGML_LOG_INFO("kleidiai: primary q8 kernel feature %s\n", cpu_feature_to_string(ctx.kernels_q8->required_cpu));
        }

        // SME thread cap should reflect actual SME enablement
        ctx.sme_thread_cap = (ctx.features & CPU_FEATURE_SME) ? sme_cores : 0;

        //printf("kleidiai: primary q4 kernel feature %s(%x) detected smcus %d(%d)\n",
        //                  cpu_feature_to_string(ctx.kernels_q4->required_cpu), ctx.kernels_q4->required_cpu, (int)detect_num_smcus(), sme_cores);
        // Helpful summary log (no printf, no null deref)
...
static inline bool kleidiai_pack_fallback_allowed() {
    // If SME cap is not set or SME is disabled, no fallback packing.
    if (ctx.sme_thread_cap <= 0) {
        return false;
    }
    // If threads hint is not provided, be conservative and pack all slots.
    if (ctx.thread_hint <= 0) {
        return true;
    }
    // Only pack fallback if the expected threads exceed SME capacity.
...
static inline ggml_kleidiai_kernels * kleidiai_primary_kernel() {
...
static int kleidiai_collect_kernel_chain(const struct ggml_tensor * op,
    ...
    if ((primary->required_cpu & CPU_FEATURE_SME) == CPU_FEATURE_SME) {
        cpu_feature fallback_mask = static_cast<cpu_feature>(ctx.features & ~CPU_FEATURE_SME);
        if (fallback_mask != CPU_FEATURE_NONE) {
            ggml_kleidiai_kernels * fallback = ggml_kleidiai_select_kernels(fallback_mask, op);
#if KAI_DEBUG_PRINT
            if (fallback) {
                printf("kleidiai collect: fallback feature=%d\n", fallback->required_cpu);
            } else {
                printf("kleidiai collect: no fallback for mask=%d\n", fallback_mask);
            }
#endif
...
static int kleidiai_collect_q4_chain(std::array<ggml_kleidiai_kernels *, GGML_KLEIDIAI_MAX_KERNEL_SLOTS> & out) {
    int count = 0;
    ggml_kleidiai_kernels * primary = kleidiai_primary_kernel();
    if (!primary) {
        return 0;
    }
    out[count++] = primary;
    ...
    if ((primary->required_cpu & CPU_FEATURE_SME) == CPU_FEATURE_SME) {
        cpu_feature fallback_mask = static_cast<cpu_feature>(ctx.features & ~CPU_FEATURE_SME);
        if (fallback_mask != CPU_FEATURE_NONE) {
            ggml_kleidiai_kernels * fallback = ggml_kleidiai_select_kernels_q4_0(fallback_mask);
#if KAI_DEBUG_PRINT
            if (fallback) {
                printf("kleidiai q4 chain: fallback feature=%d\n", fallback->required_cpu);
            } else {
                printf("kleidiai q4 chain: no fallback for mask=%d\n", fallback_mask);
            }
#endif
            if (fallback && fallback != primary &&
                fallback->lhs_type == primary->lhs_type &&
...
static int kleidiai_collect_q8_chain(std::array<ggml_kleidiai_kernels *, GGML_KLEIDIAI_MAX_KERNEL_SLOTS> & out) {
    ...
    if ((primary->required_cpu & CPU_FEATURE_SME) == CPU_FEATURE_SME) {
        cpu_feature fallback_mask = static_cast<cpu_feature>(ctx.features & ~CPU_FEATURE_SME);
        if (fallback_mask != CPU_FEATURE_NONE) {
            ggml_kleidiai_kernels * fallback = ggml_kleidiai_select_kernels_q8_0(fallback_mask);
#if KAI_DEBUG_PRINT
            if (fallback) {
                printf("kleidiai q8 chain: fallback feature=%d\n", fallback->required_cpu);
            } else {
                printf("kleidiai q8 chain: no fallback for mask=%d\n", fallback_mask);
            }
#endif
            if (fallback && fallback != primary &&
                fallback->lhs_type == primary->lhs_type &&
...
class tensor_traits : public ggml::cpu::tensor_traits {
    bool work_size(int /* n_threads */, const struct ggml_tensor * op, size_t & size) override {
        ...
        if (op->src[0]->type == GGML_TYPE_Q4_0) {
            ...
            for (int slot = 0; slot < slot_count; ++slot) {
                ...
                const size_t packed = lhs_info->packed_size_ex(m, k, QK4_0, mr, kr, sr);
        ...
        if (op->src[0]->type == GGML_TYPE_Q8_0) {
            ...
            for (int slot = 0; slot < slot_count; ++slot) {
                ...
                const size_t kr = kernel->get_kr();
                const size_t sr = kernel->get_sr();
                const size_t packed = lhs_info->packed_size_ex(m, k, QK8_0, mr, kr, sr);
                cursor   = align_up(cursor, GGML_KLEIDIAI_PACK_ALIGN);
                cursor  += packed;
                any_slot = true;
            }
    ...
    bool compute_forward_qx(struct ggml_compute_params * params, struct ggml_tensor * dst) {
        ...
        const bool too_small_for_hybrid = (min_cols_per_thread < 8) || (ne11 < 8);
        ...
        const bool is_q8 = src0->type == GGML_TYPE_Q8_0;
        uint64_t weighted_total = 0;
        if (runtime_count > 1 && is_q8 && sme_slot != -1) {
            for (int i = 0; i < runtime_count; ++i) {
                const uint64_t weight = (i == sme_slot) ? 4 : 1;
...
#if KAI_DEBUG_PRINT
        if (ith_total == 0) {
            printf("kleidiai q4_0: nth=%d sme_cap=%d runtime=%d use_hybrid=%d\n",
                   nth_total, sme_cap_limit, runtime_count, use_hybrid ? 1 : 0);
            for (int i = 0; i < runtime_count; ++i) {
                printf("  slot %d (cpu=%d) threads=%d columns=%zu\n",
                       i,
                       runtime[i].kernels->required_cpu,
                       runtime[i].assigned_threads,
                       runtime[i].n_cols);
            }
        }
#endif
...
#if KAI_DEBUG_PRINT
                        if (nth_total > 1) {
                            printf("thread %d -> slot %d (cols=%zu)\n", ith_total, local_slot, cols);
                        }
#endif
                        const ggml_type slot_rhs_type = slot.kernels->rhs_type;
                        const size_t slot_lhs_exec_arg = slot_rhs_type == GGML_TYPE_Q4_0 ? QK4_0 :
...
#if KAI_DEBUG_PRINT
        printf("kleidiai repack: slot_count=%u\n", header->slot_count);
#endif
